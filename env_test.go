...
func TestBool(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "Environment variable set to T",
			envValue:  "T",
			fallback:  false,
			wantValue: true,
		},
		{
			name:      "Environment variable set to F",
			envValue:  "F",
			fallback:  true,
			wantValue: false,
		},
		...
		{
			name:      "Environment variable is foobar",
			envValue:  "foobar",
			fallback:  true,
			wantValue: true,
		},
...
func TestInt(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "Environment variable is positive",
			envValue:  "42",
			fallback:  43,
			wantValue: 42,
		},
		{
			name:      "Environment variable is negative",
			envValue:  "-42",
			fallback:  42,
			wantValue: -42,
		},
...
func TestInt64(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "Environment variable is 64-bit positive",
			envValue:  "9223372036854775807",
			fallback:  42,
			wantValue: 9223372036854775807,
		},
		{
			name:      "Environment variable is 64-bit negative",
...
func TestFloat32(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "Environment variable is zero zero",
			envValue:  "0.0",
			fallback:  4.2,
			wantValue: 0,
		},
		{
			name:      "Environment variable is fraction",
			envValue:  "0.2",
			fallback:  0.3,
			wantValue: 0.2,
		},
		{
			name:      "Environment variable is negative fraction",
			envValue:  "-0.2",
			fallback:  0.3,
			wantValue: -0.2,
		},
...
func TestDuration(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "Environment variable is nano",
			envValue:  "2ns",
			fallback:  2 * time.Second,
			wantValue: 2 * time.Nanosecond,
		},
		{
			name:      "Environment variable is micro",
			envValue:  "2μs",
			fallback:  2 * time.Second,
			wantValue: 2 * time.Microsecond,
		},
		{
			name:      "Environment variable is millis",
			envValue:  "2ms",
			fallback:  2 * time.Second,
			wantValue: 2 * time.Millisecond,
		},
		{
			name:      "Environment variable is composite",
			envValue:  "2h30m45s",
